...
CONFIG_FILE=${SERVERFILES_FOLDER}/UnturnedServer_Data/MonoBleedingEdge/etc/mono/config
LSGMuntserverCFG=${BASEPATH}/lgsm/config-lgsm/untserver/untserver.cfg
SERVER_CONFIG=${SERVERFILES_FOLDER}/untserver.xml
...
echo "[Undead Legacy] Adding missing dll to UnturnedServer_Data/MonoBleedingEdge/etc/mono/config"
...
if grep -q "untserver.xml" $SERVERFILES_FOLDER/run_bepinex_server.sh
    then
        echo "[Undead Legacy] Skiping config file changes, already replaced"
    else
        sed -i '/.*config_file="serverconfig.xml".*/ s/serverconfig.xml/untserver.xml/' $SERVERFILES_FOLDER/run_bepinex_server.sh
fi

echo "[Undead Legacy] Replacing Monitoring start time for LinuxGSM"

if grep -q "querydelay" $LSGMuntserverCFG
    then
        sed -i 's/querydelay=.*/querydelay="10"/' $LSGMuntserverCFG
    else
        echo querydelay='"10"' >> $LSGMuntserverCFG
fi
...
if grep -q "executable" $LSGMuntserverCFG
    then
        sed -i 's/executable=.*/executable=".\/run_bepinex_server.sh"/' $LSGMuntserverCFG
    else
        echo executable='"./run_bepinex_server.sh"' >> $LSGMuntserverCFG
fi
